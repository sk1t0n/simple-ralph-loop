...
while [ -s "$TASK_FILE" ]; do
  ...
  # Выводим информацию о выполняющейся задаче в терминал и записываем её в лог
  echo "------------------------------------------------------" | tee -a "$LOG_FILE"
  echo "Задача $CURRENT_TASK_NUM из $TOTAL_TASKS: $PROMPT" | tee -a "$LOG_FILE"
  echo "------------------------------------------------------" | tee -a "$LOG_FILE"
  ...
  if [ "$VERBOSE" = true ]; then
    ...
    # Сохраняем статус сразу, чтобы последующие команды его не затёрли
    STATUS=${PIPESTATUS[0]}
  else
  ...
  # Записываем результат выполнения в лог
  {
    echo
